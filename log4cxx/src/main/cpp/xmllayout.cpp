     ...
     Pool& p) const
{
        ...
        if (properties) {
            LoggingEventDetail::KeySet propertySet(event->getPropertyKeySet());
            LoggingEventDetail::KeySet keySet(event->getMDCKeySet());
            if (!(keySet.empty() && propertySet.empty())) {
                output.append(LOG4CXX_STR("<log4j:properties>"));
                output.append(LOG4CXX_EOL);
                for (LoggingEventDetail::KeySet::const_iterator i = keySet.begin();
                        i != keySet.end(); 
                        i++) {
                        LogString key(*i);
                        LogString value;
            ...
            for (LoggingEventDetail::KeySet::const_iterator i2 = propertySet.begin();
                        i2 != propertySet.end(); 
                        i2++) {
                        LogString key(*i2);
                        LogString value;
